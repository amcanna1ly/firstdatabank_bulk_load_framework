/* 03_FDB_Insert_Config_From_Directories.sql
   Scans same folders and INSERTs into dbo.FDB_BulkLoadConfig.
   TargetSchema = 'dbo' and TargetTable = FileName by default.
*/
...
BEGIN
    ...
    EXEC master.sys.xp_dirtree @ScanPath, 1, 1;
...
-- Insert into config, avoiding duplicates
INSERT INTO dbo.FDB_BulkLoadConfig
    (Category, RelativePath, FileName, TargetSchema, TargetTable, FieldTerminator, RowTerminator)
SELECT
    f.Category,
    f.RelativePath,
    f.FileName,
    N'dbo' AS TargetSchema,
    f.FileName AS TargetTable,
    N'|' AS FieldTerminator,
    N'\n' AS RowTerminator
FROM #Files f
WHERE NOT EXISTS (
    SELECT 1
    FROM dbo.FDB_BulkLoadConfig c
    WHERE c.Category     = f.Category
      AND c.RelativePath = f.RelativePath
